...
//        }
        ...
        //1;
        List<MarkSummary> summaryList = null;
        ...
        if (!Util.isFieldEmpty(filter)) {
            ...
            if (filter.equalsIgnoreCase("pass")) {
                filterNum = 1;
                summaryList = MarkSummary.getSummary(filterNum, start, amount);
            } else if (filter.equalsIgnoreCase("alternate")) {
                filterNum = 2;
                summaryList = MarkSummary.getSummary(filterNum, start, amount);
            } else if (filter.equalsIgnoreCase("fail")) {
                filterNum = 3;
                summaryList = MarkSummary.getSummary(filterNum, start, amount);               
            } else {
                summaryList = MarkSummary.getSummary(start, amount);
            }
        } else {
            summaryList = MarkSummary.getSummary(start, amount);
        }

        // Retrieve size of summary list
        int summarySize = MarkSummary.getSummarySize(filterNum);
        
        // Forward result to summary.jsp
        request.setAttribute("currentPage", page);
        request.setAttribute("amountOfPages", (int) Math.ceil(summarySize / (amount * 1.0)));
        request.setAttribute("summarySize", summarySize);
        request.setAttribute("sumFullMark", Question.getSumFullMark());
        request.setAttribute("summaryList", summaryList);
        getServletContext().getRequestDispatcher("/WEB-INF/jsp/summary.jsp").forward(request, response);
    }
